...
잘못된 값 및 이상치 수정 과정
- 문자열 포함 여부 확인: 데이터 타입을 숫자로 변환할 때 불필요한 문자가 섞여 있으면 오류가 발생한다. 이때 판다스 시리즈 객체의 str 속성 아래 있는 contains 메서드를 활용한다. 여기에 정규 표현식을 넣으면 숫자가 아닌 문자가 섞인 데이터를 쉽게 찾아낼 수 있다.
- 정규 표현식을 통한 정제: replace 메서드에 regex 매개변수를 참으로 설정하여 정규 표현식을 적용하면, 혼합된 문자열 안에서 연도를 나타내는 숫자 패턴만 그룹으로 묶어 추출하는 등 복잡한 문자열을 깔끔하게 정제할 수 있다.
- 안전한 데이터 타입 변환: 문자열 정제 후에도 온전한 숫자로 변환할 수 없는 알 수 없는 값들은 마이너스 1과 같은 임의의 숫자로 채워 넣는다. 그 후 astype 메서드를 사용해 비로소 전체 열을 정수형으로 안전하게 변환한다.
- 비교 연산을 통한 이상치 보정: gt 메서드처럼 대소 관계를 비교하는 판다스 메서드를 사용하여 발행 연도가 비정상적으로 큰 이상치 데이터를 찾아낸다. 부등호 기호를 직접 쓰는 것과 동일한 역할을 한다.
- 최종 데이터 보정: 찾아낸 이상치 중 단기 연도처럼 특정 규칙이 있는 값은 일괄적인 뺄셈 연산을 통해 서기 연도로 정상화한다. 여전히 이상한 값들은 모두 마이너스 1로 통일하여 분석에 방해가 되지 않도록 데이터를 마무리한다.
